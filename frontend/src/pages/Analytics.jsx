...
function SectionCard({ title, subtitle, badge, children, loading }) {
  return (
    <div className="card p-5">
      ...
      {loading ? <div className="skeleton w-full h-56 rounded-lg" /> : children}
    </div>
  );
}
  ...
  async function loadAll() {
    ...
    try {
      const [rev, cats, regs, prods, custs] = await Promise.all([
        getRevenue(granularity, granularity === "monthly" ? 24 : 60),
        getCategories(),
        getRegions(),
        getTopProducts(10),
        getTopCustomers(10),
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="card p-5">
          ...
          {loading ? <div className="skeleton w-full h-56 rounded-lg" /> : <RevenueChart data={revenue} />}
        </div>

        {/* Category + Region */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <SectionCard title="Sales by Category" subtitle="Revenue per product category" loading={loading}>
        ...
        {/* Top products */}
        <SectionCard title="Top 10 Products" subtitle="By total revenue" badge="Top 10" loading={loading}>
          <TopProductsChart data={topProducts} />
        </SectionCard>

        {/* Top customers table */}
        <div className="card overflow-hidden">
          <div className="p-5 border-b border-canvas-border">
            <h2 className="text-sm font-semibold text-white">Top 10 Customers</h2>
            <p className="text-xs text-slate-500 mt-0.5">Highest spending accounts</p>
          </div>
          {loading ? (
                ...
                <tbody>
                  {topCustomers.map((c, idx) => (
                    <tr key={c.customer_name}>
                      <td className="text-slate-500 tabular-nums">{idx + 1}</td>
